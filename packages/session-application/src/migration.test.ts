...
test('a marked store absorbs legacy residue once: history, watch edges, pending debt, and envelopes are migrated then retired', () => {
  ...
  assert.deepEqual({ ...report.residue, sourceDigest: undefined, backupRoot: undefined }, { records: 1, events: 6, parentEdges: 1, watchEdges: 1, pending: 1, unclaimed: ['timeline-parent-test'], sourceDigest: undefined, backupRoot: undefined })
  ...
  assert.deepEqual(app.topology.parents('unseen', 'parent').map(edge => edge.fromSessionId), ['live'])
