...
prompt carries a runnable reply path over that same tunnel.`, ['project-bound']],
    ...
    files: [['spex session files add <path>', 'spex session files ls [SEL]', 'spex session files get <SEL> <name|path> [-o <file>]', 'spex session files ls|get --ssh <address> <FULL-SESSION-ID> [<name|path>] [-o <file>]', 'spex session files retract <path>'],
      'add and retract publish or withdraw YOUR session’s live file paths: an absolute path stored beside the session record, no bytes copied; the dashboard downloads it only when the human clicks. ls and get READ any session’s list — a parent reads what its child posted by selector (no selector = your own), and get writes the bytes of one posted file (named by its [[file:<name>]] tail or its path) to stdout or -o. --ssh addresses a session on a peered machine by full id; the path stays that machine’s, the bytes come through the tunnel.'],
...
owner — the reverse edge: a file's GOVERNORS (code: — drives drift) and REFERENCERS (related:
...
path. Bytes go to stdout by default (pipe-friendly); -o writes a file.`,
    see: 'spex issue open --evidence <hash>',
  },
