...
test('session noun-verb help projects the exact verb from the shared drawer definition', () => {
  const cases = [
    ...
    { verb: 'new', usage: 'Usage: spex session new "<prompt>"', behavior: /--prompt-file[\s\S]*successful receipt[\s\S]*PROJECT-BOUND WRITES/ },
    { verb: 'ls', usage: 'Usage: spex session ls [SEL…] [--children[=<PARENT-SEL>]]', behavior: /spex session ls --ssh <address> <FULL-SESSION-ID>[\s\S]*--children scopes it to the caller[\s\S]*Closed records are hidden[\s\S]*--all includes them[\s\S]*anchors one remote project[\s\S]*SEL = session id/ },
