...
test('--harness seeds hook nodes only when a selected native adapter can emit their events', { skip: !gitAvailable() && 'git not available' }, () => {
  ...
  // Last moved by the issue-driven-development skill node, landing beside the atlas and review-report
  // skills; a skill reaches every harness, so one new one raises all three counts together — which is what
  // separates it from a core hook node, whose arrival moves only the selections whose adapter can emit its
  // events.
