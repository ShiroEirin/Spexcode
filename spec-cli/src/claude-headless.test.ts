...
test('claude-headless is a fifth adapter with Claude materialization and a replaced runtime half', async () => {
  ...
  assert.equal(claudeHeadlessHarness.liveness({ session: 'abc' }, true, undefined, { pidAlive: true }), 'online')
  assert.equal(claudeHeadlessHarness.liveness({ session: 'abc' }, true, undefined, { pidAlive: false }), 'offline')
