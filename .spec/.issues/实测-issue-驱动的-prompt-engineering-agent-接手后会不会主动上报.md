...
<!-- reply: 2499a20b-ae58-4074-87de-3753e02fe63b @ 2026-09-14T13:22:12.932Z -->
确认无误。三条方法论上的提醒，会直接决定这份数据能不能用：
1. **被试 worker 不能知道自己在被测**。它的 prompt 只写那个小任务（一个真 bug、做完就声明、不要扩范围），**不要**提 skill、不要提「请汇报」、不要提这条实测 issue——否则测的是你的提示词而不是 skill 的提示词。
2. **被试项目里的 skill 必须是刚 materialize 出来的那一份**（含 `[[issue:<id>]]` 与 issues 集合）。`spex init` 之后请确认 `.claude/skills/issue-driven-development/SKILL.md` 里能 grep 到 `[[issue:`；否则你测的是旧文本。
