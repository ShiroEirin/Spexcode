...
- **One file per thread.** The file is a one-line `concern` plus a prose body plus appended replies —
  ...
  the thread.
- **Own lifecycle status**, store-authored never git-derived: `open` → `landed`. A legacy terminal `rejected`
  value is read as `landed`, so old decisions remain closed instead of reappearing in the open drain.
