...
- **The Issues page shows it in three places ([[issues-view]]).** The LIST row's trailing meta carries a
  ...
  verb. Below a hairline in the SAME section, the voices not in the fleet: the originator tagged `opened` (a fleet row that
  filed the issue carries the tag itself), then every other reply author tagged `replied` — each in the one session vocabulary (headline + status dot while on the board; archived name +
