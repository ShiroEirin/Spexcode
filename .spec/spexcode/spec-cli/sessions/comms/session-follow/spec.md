...
backend only appends the event; it does not attempt delivery on a channel it does not own. The adopter's post-commit
wake carries the affected watcher ids and, in whichever process made the commit, immediately translates each
cursor-addressed event into one ordinary watcher prompt and its watcher-timeline message event. Handing that prompt
to the watcher's harness is the channel owner's work: a commit inside that backend drains the queue in place, while a
session declaring from its own CLI asks that backend to drain and returns without waiting on any harness — only a
refused connection, proving no backend is there, makes the CLI drain the queue itself ([[remote-client]]). The patrol
is only the recovery path. So consecutive changes arrive
