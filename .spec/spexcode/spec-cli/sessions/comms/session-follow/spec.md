...
wake carries the affected watcher ids, immediately translates each cursor-addressed event into one ordinary watcher
prompt and its watcher-timeline message event, then drains the existing runtime; the patrol is only the recovery
path. So consecutive changes arrive
