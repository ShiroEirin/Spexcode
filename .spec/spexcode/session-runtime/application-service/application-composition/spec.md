...
The application also exposes a read-only event watermark: `readChangedSessionIdsSince` uses the append-only event table's database-local rowid to return distinct subjects added after a caller's cursor and advances that cursor in the same read transaction. This is an advisory bridge for external database watchers; callers must still fall back to a full projection when the event stream cannot account for a change.
