...
`replyVia:"note"` intent; the backend derives `harness` from that selected profile.
...
client also sends the create request's explicit `replyVia:"note"` intent so the first agent turn uses the same
