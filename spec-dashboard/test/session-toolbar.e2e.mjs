...
const pickerOpened = await page.locator('.si-rp-fab[data-action="resource-picker"]').click().then(() => true).catch(() => false)
...
const sessionTabMenu = await page.locator('[role="menu"]').last().locator('[role="menuitem"]').evaluateAll((els) => els.map((el) => el.textContent.trim())).catch(() => [])
  check('session tab context menu is the ordinary tab menu', JSON.stringify(sessionTabMenu) === JSON.stringify(['Pin', 'Close', 'Close others', 'Split right', 'Split down']), sessionTabMenu)
