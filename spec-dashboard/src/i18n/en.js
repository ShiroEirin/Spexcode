export default {
  ...
  plugins: {
    ...
    title: 'Plugins',
    count: ({ shown, total }) => (shown === total ? `${total} plugins` : `${shown} of ${total} plugins`),
    searchHint: 'filter by name or what it does',
    surfaceFilter: 'surface',
    filter: { all: 'all', hook: 'hooks', system: 'always on', invoked: 'invoked' },
    listLabel: 'plugins, by the surface they plug into',
    noMatch: 'nothing here matches that.',
    alwaysOn: 'Always on',
    invoked: 'Invoked on demand',
    frame: { turn: 'each turn', tool: 'each tool call' },
    frameElse: 'elsewhere',
    repeats: 'repeats',
    ledger: ({ runs, since }) => `${runs} hook runs since ${since}`,
    runWindow: ({ since, days }) => `counted from the dispatcher's own ledger — every run of every hook in this project since ${since} (${days} day(s) on file)`,
    neverTip: ({ since }) => `never dispatched since ${since} — it is installed and has not run once`,
    refusedTip: ({ n }) => `allowed to refuse this event, and has refused ${n} time(s) in the counted window`,
    never: 'never run',
    runRuns: 'dispatched',
    runNever: 'never — not once in the window',
    runRefused: 'refused',
    runFailed: 'failed',
    runUnfinished: 'killed mid-run',
    runMedian: 'median',
    runMs: ({ n }) => `${n}ms`,
    runLast: 'last run',
    runWhy: 'last refusal',
    foldedIn: 'folded into the agent',
    mayInvoke: 'the agent may invoke',
    toolRuns: 'the tool runs',
    waiting: 'waiting on the human',
    refusedBack: 'refused → the turn goes on',
    pickOne: 'Pick a plugin to read what it actually does.',
    loading: 'loading…',
    openNode: 'open the spec node',
    factEvents: 'binds',
    factOrder: 'order',
    factBlock: 'may refuse',
    factOff: ({ profile }) => `off under profile ${profile}`,
    factTools: 'tools',
    truncated: ({ bytes }) => `shown to the first 64KB of ${bytes} — open the file for the rest`,
  ...
  reviewList: {
    ...
    fleetNone: 'None',
    facetSub: 'Sub-issues',
    subTop: 'Fold into parent',
    subAll: 'Show all',
    facetGroup: 'Group',
    groupNone: 'No grouping',
    groupParent: 'Under parent',
  ...
  detail: {
    ...
    sideParticipants: 'on the thread',
    sideParent: 'parent',
    sideRelations: 'relations',
    relBlockedBy: 'blocked by',
    relBlocks: 'blocks',
    relRelated: 'related',
    relDuplicateOf: 'duplicate of',
    relDuplicatedBy: 'duplicated by',
  },
  ...
  fleet: {
    ...
    newWorkerTitle: 'type @new: into the reply — pick a launcher there, and your send dispatches the worker',
    merge: 'Merge',
  ...
  sessionWindow: {
    ...
    closeWorkingDetail: 'Your close request is in progress. Please wait for the result.',
    closeSucceeded: 'close confirmed',
    closeSyncing: 'The session is archived. You can restore it from the archive.',
  ...
  mobile: {
    ...
    statusNotices: ({ count }) => `${count} status notifications`,
    you: 'you',
