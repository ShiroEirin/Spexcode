export default {
  ...
  specView: {
    ...
    versionOf: '第 {n} 版，共 {total} 版',
    versionFace: '这一版的查看方式',
    versionText: '全文',
    versionChange: '本版变更',
    versionShowText: '查看这一版的全文',
    versionShowChange: '查看这一版改了什么',
    versionBack: '回到当前版本（v{n}）',
    versionMissing: '打不开这个版本：{error}',
  },
  fileView: { none: '未选择文件' },
  fileTree: {
    aria: '项目文件树',
  ...
  contextDock: {
    ...
    noHistory: '尚无版本',
    versionCurrent: 'v{n}（当前版本）',
    versionText: '查看 v{n} 全文',
    versionChange: '查看 v{n} 的变更',
  },
  empty: {
    title: '当前没有打开的文档',
  ...
  plugins: {
    ...
    title: '插件',
    count: ({ shown, total }) => (shown === total ? `${total} 个插件` : `${total} 中的 ${shown} 个`),
    searchHint: '按名字或功用筛',
    surfaceFilter: '按面筛选',
    filter: { all: '全部', hook: '钩子', system: '始终生效', invoked: '按需调用' },
    listLabel: '按所插入的面列出的插件',
    noMatch: '没有匹配的。',
    alwaysOn: '始终生效',
    invoked: '按需调用',
    frame: { turn: '每一轮', tool: '每次工具调用' },
    frameElse: '其它',
    repeats: '循环',
    ledger: ({ runs, since }) => `${since} 起 ${runs} 次钩子运行`,
    runWindow: ({ since, days }) => `取自派发器自己的流水账 —— 本项目 ${since} 以来每个钩子的每一次运行（在册 ${days} 天）`,
    neverTip: ({ since }) => `${since} 以来一次都没被派发过 —— 装着，但没跑过`,
    refusedTip: ({ n }) => `它被允许拒绝这个事件，并且在统计窗口内拒了 ${n} 次`,
    never: '没跑过',
    runRuns: '被派发',
    runNever: '从未 —— 窗口内一次都没有',
    runRefused: '拒绝',
    runFailed: '失败',
    runUnfinished: '跑到一半被杀',
    runMedian: '中位耗时',
    runMs: ({ n }) => `${n}ms`,
    runLast: '最近一次',
    runWhy: '最近一次拒绝的理由',
    foldedIn: '折进 agent 的上下文',
    mayInvoke: 'agent 可以调用',
    toolRuns: '工具执行',
    waiting: '等人',
    refusedBack: '被拒 → 这一轮继续',
    ...
    factOrder: '顺序',
    factBlock: '可拒绝',
    factOff: ({ profile }) => `在 profile ${profile} 下已禁用`,
    factTools: '工具',
    truncated: ({ bytes }) => `共 ${bytes} 字节，只显示前 64KB —— 其余请打开文件`,
    failed: ({ reason }) => `读不到插件面：${reason}`,
  },
  nav: {
    railLabel: '主导航',
    plugins: '插件',
  ...
  projects: {
    ...
    thisMachine: '本机',
    machineOffline: '离线',
    machineNoGatewayShort: '无 gateway',
    machineNoGateway: '这台机器已连接，但没有可访问的 gateway — 它的 agent 通道不受影响。',
    machineUnreachable: '刚才读不到那台机器的项目列表。',
    machineDenied: '那台机器拒绝了本 gateway 的凭据。',
    ...
    initGit: '初始化 Git 仓库',
    initSpex: '初始化 SpexCode',
    spexInitialized: '已初始化 SpexCode',
    harnessTargets: 'harness 目标',
    alreadyAdded: '已在项目目录中',
    configTitle: '编辑 .spec/spexcode.json',
    configEditor: '.spec/spexcode.json 内容',
    configLoading: '加载中…',
    configSave: '保存',
    configSaving: '保存中…',
    configReload: '重新加载',
    ...
    projectIcon: '项目图标',
    editGatewayIcon: '编辑 gateway 图标',
    editProjectIcon: '编辑项目图标',
    gatewaySettings: 'gateway 设置',
    iconSearch: '搜索图标',
    iconSources: '图标来源',
    iconSourceFeatured: '精选',
    ...
    removeTitle: '移除项目登记？',
    removeWarning: ({ name }) => `确认移除“${name}”的项目登记？本地目录、Git 历史和源码会保留。`,
    ...
    hostVersions: '工具链', hostAgents: '代理 CLI',
  ...
  detail: {
    ...
    backToParent: '返回父 issue',
    sideSubIssues: '子 issue',
    sideIssue: '议题',
    sideNode: '规格节点',
    sideSessions: '会话',
    sideParticipants: '线程上的人',
    sideParent: '父 issue',
    sideRelations: '关系',
    relBlockedBy: '被阻塞于',
    relBlocks: '阻塞',
  ...
  fleet: {
    ...
    stopped: ({ n }) => `已停 · ${n}`,
    newWorker: '新建 worker',
    newWorkerTitle: '把 @new: 打进回复框——在那里选启动器，你按发送才真正派工',
    merge: '合并',
  ...
  legend: {
    ...
    shell: {
      ...
      sessionNext: '选择下一个会话',
      sessionExpand: '展开当前会话',
      sessionCollapse: '折叠当前会话',
  ...
  sessionWindow: {
    ...
    emptyAfter: ' 开始一个',
    specRelated: '关联的 spec',
    specMore: '还有 {n} 个——在图谱上找',
    findOnGraph: '在图谱上找',
    rename: '重命名',
    attach: '在终端中打开…',
    select: '多选…',
    ...
    quarantineConfirm: '隔离',
    quarantineAdapter: '适配器',
    quarantineThread: '原生线程',
    quarantineTmux: 'tmux 会话',
    quarantineWorktree: '工作树路径',
    quarantineBranch: '分支',
    ...
    closeWorking: '正在关闭…',
    closeWorkingDetail: '关闭请求正在处理，请等待结果。',
    closeSucceeded: '关闭已确认',
    closeSyncing: '会话已归档，可以从归档中恢复。',
    closeFailed: '关闭失败',
    closeRetry: '重试关闭',
  ...
  session: {
    ...
    issuesStoreLabel: '来源',
    issuesNew: '新建',
    issuesNewTitle: '新建 issue',
    issuesTitleLabel: '标题',
    issuesBodyLabel: '正文',
    issuesWrite: '编辑',
    issuesPreview: '预览',
    issuesPreviewEmpty: '还没有可预览的内容',
    issuesNodesHint: '在正文里写 [[id]] 即可关联',
    issuesCancel: '取消',
    issuesSend: '发送',
    issuesPost: '创建',
    issuesSending: '发送中……',
    ...
    issuesSubTitle: '子 issue',
    issuesThread: ({ n }) => `动态 · ${n}`,
    issuesSubProgress: ({ closed, total }) => `已完成 ${closed}/${total}`,
    ...
    harnessLabel: '启动智能体',
    launcherLabel: '启动器',
    launcherTip: '修改 .spec/spexcode.json / .spec/spexcode.local.json 来配置启动器',
    launcherPickerTitle: '选择启动器',
    launcherPickerHint: '启动器配置位于设置中。',
    launcherSettings: '打开启动器设置',
    tabTerminal: '终端',
    tabConversation: '对话',
    tabDiff: '差异',
    ...
    diffLoading: '正在加载差异……',
    diffEmpty: '没有分支改动',
    diffNoCommits: '这个分支还没有自己的提交',
    diffGroupCommitted: '已提交',
    diffGroupUncommitted: '未提交',
    diffUnchangedLines: '{n} 行未改动',
    diffUncommitted: '未提交',
    ...
    filesTitle: '会话文件',
    filesEmptyTitle: '尚未发布会话文件',
    filesListLabel: '已发布的会话文件',
    previewFile: '预览文件',
    downloadFile: '下载文件',
    ...
    resourcePickerTitle: '文件与服务',
    resourceSearch: '按名称搜索',
    resourceSearchClear: '清空搜索',
    resourceFilterLabel: '按类型筛选',
    resourceEmptyTitle: '还没有发布任何文件',
    resourceEmptyHint: 'Agent 发布的文件、你发给它的文件，都会出现在这里。',
    resourceNoMatch: '没有匹配的文件',
    resourceUploadedTag: '用户上传',
    resourceOpenTag: '已打开',
    resourceType: {
      all: '全部', html: 'HTML', pdf: 'PDF', markdown: 'Markdown', image: '图片', text: '文本', json: 'JSON',
      csv: 'CSV', video: '视频', archive: '压缩包', web: '网页', uploaded: '用户上传', other: '文件',
    },
  ...
  settings: {
    ...
    secTerminal: '终端',
    secLaunchers: '启动器',
    launchersDescription: '启动器配置来自与 CLI 启动会话相同的项目配置链路。',
    launcherConfigPath: '另外，你也可以直接编辑 .spec/spexcode.json 做高级修改。',
    launcherName: '启动器名称',
    launcherType: 'harness 类型',
    launcherCommand: '指令',
    launcherAdd: '添加启动器',
    launcherSaving: '保存中……',
    launcherSaved: '启动器已保存。',
    launcherSaveFailed: '无法保存启动器。',
    launcherInvalidConfig: '项目配置无效。',
    noLaunchers: '尚未配置启动器。',
    secConfig: '项目配置',
    configDescription: '查看或编辑 CLI 与 host 共用的 portable .spec/spexcode.json。',
    configOpen: '打开 .spec/spexcode.json',
    configClose: '关闭编辑器',
    configEditor: '.spec/spexcode.json 内容',
    configLoading: '加载中……',
    configSaving: '保存中……',
    ...
    configReload: '重新加载',
    configSaved: '已保存。',
    configLoadFailed: '无法读取 .spec/spexcode.json。',
    configSaveFailed: '无法保存 .spec/spexcode.json。',
    configInvalid: '项目配置不是有效 JSON。',
    defaultSessionSurface: '默认会话界面',
  ...
  mobile: {
    ...
    statusNotices: ({ count }) => `${count} 条状态通知`,
    you: '我',
