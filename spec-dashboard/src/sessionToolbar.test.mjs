...
test('pane-backed and headless consoles share one warm TimelineChat Conversation surface', () => {
  ...
  assert.match(timelineChat, /sendSessionCommand\(s\.id, composed, \{\s*\n?\s*replyVia: 'note',/)
...
test('conversation revisit refreshes a warm instance without clearing its rendered state', () => {
  ...
  assert.match(timelineChat, /useEffect\(\(\) => \{\n    setEvents\(null\); setWin\(\{ stamp: null, offset: 0, total: 0, priorWorking: false \}\); setLoadingEarlier\(false\); stampRef\.current = null; anchorRef\.current = null; setDetail\(null\); setSendNote\(null\); setExpandedSeams\(new Set\(\)\); setTranscripts\(new Map\(\)\);[\s\S]*?\n  \}, \[s\.id\]\)/)
